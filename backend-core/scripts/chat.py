...
async def choose_agent() -> int:
    """登録済みエージェント一覧を表示し、ユーザーに選ばせてagent_idを返す。

    番号入力・agent_id（DB上の`agents.id`、数値）直接入力のどちらでも選べるようにする。
    ...
    while True:
        ...
        if choice.isdigit():
            value = int(choice)
            if value in agent_by_id:
                return agent_by_id[value].id
            if 1 <= value <= len(agents):
                return agents[value - 1].id
        print("入力が正しくありません。もう一度入力してください。")


async def main() -> None:
    agent_id = await choose_agent()
